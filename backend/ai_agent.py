...
def orchestrate_agent_workflow(question: str, context: Dict) -> Dict:
    ...
    # 0. Out of scope domain refusal check
    if stage_a['intent'] == 'out_of_scope_refusal':
        return {
            "answer": (
                "**Domain Boundary Notice**: Finora is an Autonomous AI Financial Controller specialized strictly in corporate ledger auditing, continuous 3-way reconciliation, statutory tax compliance (Ind AS, GSTR-2B, TDS), and treasury cash forecasting. I cannot provide assistance on non-financial topics."
            ),
            "confidence": "HIGH",
            "confidence_score": 1.0,
            "confidence_rationale": "Strict zero-hallucination domain fence active.",
            "escalation_recommendation": None,
            "reasoning_trail": reasoning_trail,
            "verifier_passed": True
        }

    # 1. PRIMARY NEURAL DISPATCH: Route directly to local on-device Gemma 3 with full 3-layer Multi-Brain context
    gemma_res = query_local_gemma3(question, context)
    if gemma_res and gemma_res.get("answer") and len(gemma_res["answer"]) > 25:
        reasoning_trail.append({
            "step_number": 3,
            "action": f"Dispatched query to On-Device Neural Gemma 3 ({', '.join(gemma_res.get('brains_engaged', []))})",
            "tool": "local_gemma3_ollama",
            "input": {"prompt": question, "brains": gemma_res.get("brains_engaged")},
            "observation": f"Generated {gemma_res.get('token_count', 0)} tokens in {gemma_res.get('elapsed_sec', 0):.2f}s with 3-layer live grounding."
        })
        ...
        return {
            ...
            "confidence_rationale": f"Generated by on-device Gemma 3 ({', '.join(gemma_res.get('brains_engaged', []))}) with full 3-layer database, statutory standard, and viewport grounding.",
            "escalation_recommendation": None,
            "evidence_trail": reasoning_trail,
            "reasoning_trail": reasoning_trail,
            "verifier_passed": True,
            "brains_engaged": gemma_res.get("brains_engaged"),
            "is_neural_llm": True,
            "elapsed_sec": gemma_res.get("elapsed_sec")
        }
